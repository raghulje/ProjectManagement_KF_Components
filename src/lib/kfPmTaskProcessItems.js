...
function mapRowsLight(rows) {
  return dedupeRawRows(rows)
    .map((row, idx) => mapAdminTaskRow(row, idx))
    .filter(Boolean);
}
...
export async function fetchMyCreatedTasksByStatus(
  ...
  const rows = mapRowsLight(extractListPayload(response));
...
export async function fetchAssignedOpenProcessTasks(
  ...
  return { rows: mapRowsLight(batches.flat()), total, page: pn, pageSize: ps };
}
